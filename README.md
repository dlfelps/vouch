...
$ vouch ls resnet           # or search from the terminal
$ vouch trace evaluate.cifar.resnet.acc
```
...
| `vouch ls` / `vouch trace` / `vouch status` | find keys, see where a value came from, see which runs are stale |
